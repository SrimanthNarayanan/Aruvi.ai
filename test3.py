...
def display_document_analysis_page():
    ...
    if analysis:
        ...
        if st.session_state.email_sent:
            st.info("📧 Email sent successfully! You can send another email if needed.")
            st.button("🔄 Send Another Email", key="send_another_doc_email", on_click=reset_email_state)
        else:
            # --- START: FIX BY USING st.form for Email ---
            with st.expander("Email Document Analysis", expanded=False):
                with st.form(key="doc_email_form"):
                    col_email_doc, col_format_doc = st.columns([0.6, 0.4])
                    with col_email_doc:
                        recipient_email = st.text_input("Recipient Email", value=st.session_state.recipient_email or "", key="f_doc_email")
                    with col_format_doc:
                        email_format = st.selectbox("Attachment Format", ["PDF Report"], key="f_doc_email_format")
                        
                    email_message = st.text_area("Email Message (Optional)", value=st.session_state.email_message or "Please find the attached document analysis report.", key="f_doc_email_message")
